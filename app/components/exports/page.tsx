...
const FORMAT_META: Record<ExportFormat, { icon: React.ElementType; label: string; desc: string; color: string; bg: string }> = {
  csv: { icon: Table, label: "CSV", desc: "Comma-separated values for spreadsheets", color: "text-success", bg: "bg-success/10 border-success/20" },
  pdf: { icon: FileText, label: "PDF", desc: "Formatted report ready to share or print", color: "text-error", bg: "bg-error/10 border-error/20" },
  api: { icon: Code2, label: "REST API", desc: "Stream data via authenticated endpoint", color: "text-primary", bg: "bg-primary/10 border-primary/20" },
  excel: { icon: File, label: "Excel", desc: "Native .xlsx with formulas & charts", color: "text-warning", bg: "bg-warning/10 border-warning/20" },
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-lg bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center">
                <FileDown className="w-4 h-4 text-success" />
              </div>
              <div>
                <h1 className="text-sm font-bold text-text-primary">Exports</h1>
                <p className="text-[10px] text-text-muted">CSV · PDF · REST API · Excel · JSON</p>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-semibold text-success bg-success/10 border border-success/20 px-2 py-1 rounded-full">5 formats</span>
            <span className="text-[10px] font-semibold text-primary bg-primary/10 border border-primary/20 px-2 py-1 rounded-full">{RECENT_EXPORTS.filter(e => e.status === "completed").length} recent exports</span>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="bg-bg-surface border border-border-subtle rounded-2xl p-5">
              <h2 className="text-sm font-bold text-text-primary mb-3 flex items-center gap-2">
                <Download className="w-4 h-4 text-success" /> Choose Export Format
              </h2>
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center gap-2 p-3 rounded-xl border transition-all ${
                        selectedFormat === f ? `${bg} border-current ${color} shadow-sm` : "bg-bg-elevated border-border-subtle text-text-muted hover:text-text-primary hover:border-border-default"
                      }`}
                    >
              ...
              <p className="text-xs text-text-muted mt-3">{fmt.desc}</p>
            </div>
            ...
            {selectedFormat !== "api" ? (
              <div className="bg-bg-surface border border-border-subtle rounded-2xl p-5 space-y-4">
                <h2 className="text-sm font-bold text-text-primary flex items-center gap-2">
                  <Database className="w-4 h-4 text-primary" /> Dataset & Options
                </h2>
                {/* Dataset */}
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {DATASETS.map(ds => (
                      <button
                        ...
                        className={`flex flex-col items-start px-3 py-2 rounded-xl border text-xs transition-all ${selectedDataset === ds.id ? "bg-primary/10 border-primary/30 text-primary" : "bg-bg-elevated border-border-subtle text-text-secondary hover:text-text-primary"}`}
                      >
                        <span className="font-semibold">{ds.label}</span>
                        <span className="text-[9px] text-text-muted">{ds.count}</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[["last7", "Last 7d"], ["last30", "Last 30d"], ["last90", "Last 90d"], ["ytd", "Year to Date"], ["all", "All Time"]].map(([v, l]) => (
                      <button key={v} onClick={() => setDateFilter(v)} className={`px-3 py-1.5 rounded-lg border text-xs font-semibold transition-all ${dateFilter === v ? "bg-primary/10 border-primary/30 text-primary" : "bg-bg-elevated border-border-subtle text-text-muted hover:text-text-primary"}`}>{l}</button>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-4">
                  {selectedFormat === "csv" && (
                    <label className="flex items-center gap-2 cursor-pointer">
                      <div onClick={() => setIncludeHeaders(!includeHeaders)} className={`w-8 h-4 rounded-full relative transition-colors ${includeHeaders ? "bg-primary" : "bg-border-default"}`}>
                        <div className={`absolute top-0.5 w-3 h-3 bg-white rounded-full shadow transition-all ${includeHeaders ? "left-4" : "left-0.5"}`} />
                      </div>
                      <span className="text-xs text-text-secondary">Include headers</span>
                    </label>
                  )}
                  <label className="flex items-center gap-2 cursor-pointer">
                    <div onClick={() => setCompression(!compression)} className={`w-8 h-4 rounded-full relative transition-colors ${compression ? "bg-primary" : "bg-border-default"}`}>
                      <div className={`absolute top-0.5 w-3 h-3 bg-white rounded-full shadow transition-all ${compression ? "left-4" : "left-0.5"}`} />
                    </div>
              ...
              <div className="bg-bg-surface border border-border-subtle rounded-2xl p-5 space-y-4">
                <h2 className="text-sm font-bold text-text-primary flex items-center gap-2">
                  <Code2 className="w-4 h-4 text-primary" /> REST API Access
                </h2>
                <div className="bg-bg-elevated border border-border-subtle rounded-xl p-4">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest">Your API Key</p>
                    <button onClick={copyApiKey} className={`text-[10px] font-semibold transition-colors ${copiedKey ? "text-success" : "text-primary hover:text-primary/80"}`}>
                      {copiedKey ? "Copied!" : "Copy"}
                    </button>
                  </div>
                  <p className="text-xs font-mono text-text-primary tracking-wider">prism_live_sk_a7b3c9d2e1f0g4h5i6j7k8</p>
                </div>
                <div className="bg-bg-base rounded-xl border border-border-subtle overflow-hidden">
                  ...
                  <pre className="p-4 text-[11px] font-mono text-success overflow-x-auto leading-relaxed">{API_SNIPPET}</pre>
                </div>
                <div className="grid grid-cols-3 gap-3">
                  {[["Rate Limit", "10k req/hr"], ["Avg Latency", "~120ms"], ["Uptime SLA", "99.99%"]].map(([k, v]) => (
                    <div key={k} className="bg-bg-elevated border border-border-subtle rounded-xl p-3">
                      <p className="text-[9px] text-text-muted">{k}</p>
          ...
          <div className="space-y-4">
            ...
            {selectedFormat !== "api" && (
              <div className="bg-bg-surface border border-border-subtle rounded-2xl p-5 text-center">
                ...
                <h3 className="text-sm font-bold text-text-primary mb-1">{fmt.label} Export</h3>
                ...
                <AnimatePresence mode="wait">
                  {exportStatus === "idle" && (
                    <motion.button
                      ...
                      className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-primary text-white rounded-xl font-semibold text-sm hover:bg-primary/90 transition-colors shadow-[0_0_20px_rgba(99,102,241,0.3)]"
                    >
                  ...
                  {exportStatus === "generating" && (
                    <motion.div key="generating" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
                      <div className="flex items-center gap-2 text-xs text-text-muted">
                        <RefreshCw className="w-3.5 h-3.5 animate-spin text-primary" /> Generating your export…
                      </div>
                  ...
                  {exportStatus === "ready" && (
                    <motion.div key="ready" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-3">
                      <div className="flex items-center justify-center gap-2 text-success text-xs font-semibold">
                        <CheckCircle2 className="w-4 h-4" /> Export ready!
                      </div>
                      <a
                        href="#"
                        download
                        className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-success text-white rounded-xl font-semibold text-sm hover:bg-success/90 transition-colors"
                      >
                        <Download className="w-4 h-4" /> Download {fmt.label}
                      </a>
                      <button onClick={handleReset} className="w-full text-xs text-text-muted hover:text-text-primary transition-colors">
                        New export
                      </button>
                    </motion.div>
                  )}
            ...
            <div className="bg-bg-surface border border-border-subtle rounded-2xl p-4 space-y-3">
              <h3 className="text-[10px] font-bold text-text-muted uppercase tracking-widest">Export Stats</h3>
              {[
        ...
        <div className="bg-bg-surface border border-border-subtle rounded-2xl overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-border-subtle">
            <h2 className="text-sm font-bold text-text-primary">Recent Exports</h2>
            <button className="text-xs text-primary hover:text-primary/80 transition-colors font-semibold">View all</button>
          </div>
              ...
              <thead>
                <tr className="border-b border-border-subtle bg-bg-elevated/50">
                  {["Name", "Format", "Rows", "Status", "Size", "Created", ""].map(h => (
                    <th key={h} className="px-5 py-3 text-left text-[10px] font-bold text-text-muted uppercase tracking-widest whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {RECENT_EXPORTS.map((job, i) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-5 py-3 font-semibold text-text-primary">{job.name}</td>
                      ...
                      <td className="px-5 py-3">
                        {job.status === "completed" ? (
                          <span className="flex items-center gap-1 text-success text-[10px] font-semibold"><CheckCircle2 className="w-3 h-3" /> Completed</span>
                        ) : (
                          <span className="flex items-center gap-1 text-error text-[10px] font-semibold"><AlertCircle className="w-3 h-3" /> Failed</span>
                        )}
                      </td>
                      <td className="px-5 py-3 text-text-secondary font-mono">{job.size}</td>
                      <td className="px-5 py-3 text-text-muted flex items-center gap-1 whitespace-nowrap"><Clock className="w-2.5 h-2.5" /> {job.created}</td>
                      <td className="px-5 py-3">
                        {job.status === "completed" && (
                          <button className="flex items-center gap-1 text-primary hover:text-primary/80 transition-colors font-semibold text-[10px]">
